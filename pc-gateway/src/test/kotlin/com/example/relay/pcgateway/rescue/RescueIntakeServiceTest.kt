...
class RescueIntakeServiceTest {
    ...
    fun pcDecryptsDeduplicatesCountsCarriersAndIssuesVerifiedReceipt() {
        ...
        assertEquals(ShelterReceiptStatus.STORED, service.receipt("request-1")?.receipt?.status ?: error("Receipt not found"))
    ...
    fun firstConfirmingNodeOwnsResponseAndTerminalDetailsExpireAfterThirtyDays() {
        ...
        assertEquals(ShelterReceiptStatus.ACCEPTED, service.receipt("request-1")?.receipt?.status ?: throw AssertionError("Receipt is null"))
        ...
        assertEquals(ShelterReceiptStatus.RESPONDING, service.receipt("request-1")?.receipt?.status ?: throw AssertionError("Receipt is null"))
        service.updateStatus("request-1", RescueResponseStatus.COMPLETED, "operator-a")
        assertEquals(ShelterReceiptStatus.COMPLETED, service.receipt("request-1")?.receipt?.status ?: throw AssertionError("Receipt is null"))
