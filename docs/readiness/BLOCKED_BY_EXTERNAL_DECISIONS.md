...
| Project license / OSS notice | **Resolved 2026-07-29:** the repository owner selected Apache-2.0 and this repository contains its `LICENSE` | Repository owner; every future contributor must have authority to license their contribution | Keep `LICENSE` with source and release packages; retain required third-party notices and only accept contributions that can be licensed under Apache-2.0 |
