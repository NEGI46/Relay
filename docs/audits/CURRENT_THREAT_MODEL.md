...
| Parser-fuzz input | Sends malformed/oversized frames or JSON | Decoder regression, size limits, validation, standalone JVM Jazzer targets, and a ClusterFuzzLite address-sanitized workflow | Deterministic regression is covered locally; the PR workflow exercises the real targets with bounded fuzz time. |
