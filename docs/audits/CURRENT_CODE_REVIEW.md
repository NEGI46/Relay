...
| Resolved (2026-08-01) | The job named "Parser fuzz regression" ran deterministic Python decoder/BLE regressions, not a Jazzer target. | scripts/run-jazzer.ps1 remains the deterministic lane; standalone QrFrameFuzzer and GatewayMessageFuzzer targets plus .clusterfuzzlite address-sanitized PR fuzzing now cover the real JVM decoder paths. |
...
| Resolved (2026-07-22) | Actions used movable tags such as `actions/checkout@v4`. | A tag can move; CI source integrity is weaker than a full SHA pin policy. | Workflows now pin the referenced official actions to reviewed upstream commit SHAs. Actionlint/zizmor policy and periodic SHA review remain operational work. |
| Resolved (2026-08-01) | Vulnerable historical dependency-verification entries were retained alongside current dependencies. | gradle/verification-metadata.xml now contains the current verified dependency set; OSV-Scanner reports zero findings on the repository source. | Keep metadata current with dependency updates and Dependabot changes. |
