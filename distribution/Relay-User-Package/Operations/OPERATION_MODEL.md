...
> **配布物の状態:** `Android/Relay-debug.apk` は検証用であり、本番配布物ではありません。iPhone IPA、署名・公証済みmacOSアプリ、Windows BLE bridgeのMSIXは、この配布フォルダには含まれません。実災害で使用する前に、組織のrelease署名、署名済み避難所Manifest、実機通信・復旧試験を完了してください。

> **自動提出の信頼条件:** 一般利用者の端末は鍵入力・PC選択・接続操作を行いません。地域ルート公開鍵で検証された避難所Manifestと一致するBLE bridgeだけを対象に自動提出します。署名・期限・鍵指紋・避難所IDのどれかが検証できない場合は提出せず、端末内の暗号化済み要請を保持します。
