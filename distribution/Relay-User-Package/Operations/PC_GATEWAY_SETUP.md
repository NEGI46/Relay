...
> **救助要請のゼロ操作自動提出について:** この文書にあるLAN HTTP、pairing、公開同期の手順は、運用者が行う保守・既存互換経路です。一般利用者のスマートフォンを自動提出させる経路として使ってはいけません。自動提出には、地域ルート公開鍵、署名済み避難所Manifest、当該Manifestと一致するWindows BLE bridge（MSIX）がすべて必要です。どれか一つでも欠ける・期限切れ・検証不能の場合、救助本文の受付は安全側に停止します。

> **秘密鍵の境界:** Gatewayの復号鍵と受領証明用秘密鍵はGatewayにだけ保存します。BLE bridgeへコピー、共有、環境変数で受け渡ししてはいけません。bridgeは署名済みManifestを広告・検証する接続境界です。
