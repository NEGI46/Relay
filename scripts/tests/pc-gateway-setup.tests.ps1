...
Assert-Condition ($autostart -match 'RELAY_GATEWAY_ADMIN_KEY_FILE') 'Task runner must reference the admin key file.'
Assert-Condition ($autostart -notmatch 'RELAY_GATEWAY_ADMIN_KEY\s*=') 'Task runner must not embed the admin key value.'
...
Assert-Condition ($setup -match '& \$firewallScript') 'Setup must configure the firewall through the canonical script.'
