...
if (-not (Get-Command grype -ErrorAction SilentlyContinue)) { Write-Warning 'Grype is not installed; skipping vulnerability scan.'; exit 0 }
if (-not (Test-Path -LiteralPath $InputPath)) { Write-Warning "Grype input is missing; skipping: $InputPath"; exit 0 }
