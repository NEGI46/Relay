...
$pid = & (Join-Path $PSScriptRoot 'start-pc-gateway-development.ps1') @args | Select-Object -Last 1
if($pid -isnot [int] -and "$pid" -notmatch '^\d+$'){throw 'Gateway started but a managed PID was not returned.'}
$managedProcess = Get-Process -Id ([int]$pid) -ErrorAction Stop
$stateDir=Join-Path $env:LOCALAPPDATA 'Relay\local-pilot';New-Item -ItemType Directory -Force -Path $stateDir|Out-Null
@{processId=[int]$pid;processName=$managedProcess.ProcessName;processStartedAt=$managedProcess.StartTime.ToUniversalTime().ToString('o');port=$Port;startedAt=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $stateDir 'run.json') -Encoding UTF8
