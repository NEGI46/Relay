...
class RescueWorkflowTest {
    ...
    fun `newer signed shelter status replaces an older receipt but cannot move backwards`() {
        ...
        val envelope = store.get(RescueRequestKey("request-1", 1))?.envelope ?: error("Envelope not found")
        ...
        assertEquals(RescueSubmissionStatus.SHELTER_RESPONDING, store.get(key)?.state?.submissionStatus ?: error("State is null"))
        assertEquals(
        ...
        assertEquals(RescueSubmissionStatus.SHELTER_COMPLETED, store.get(key)?.state?.submissionStatus ?: error("State is null"))
    }
    ...
    fun malformedExpiredAndFalseFramingAreRejectedWithoutEvictingValidData() {
        ...
        val valid = store.get(RescueRequestKey("request-1", 1))?.envelope
            ?: throw AssertionError("Expected envelope for 'request-1', version 1")
        assertEquals(
