...
| 配布保護 | 🧪 scripts / metadata | 🧪 TUF chain。SBOM・脆弱性・MobSF・署名検証はツール／鍵をCIへ設定するまで BLOCKED | 🚧 本番鍵・署名済み正式配布 |
...
| Fuzz | Deterministic decoder / virtual BLE regression（Jazzer targetはNOT_RUN） |
| APK scan | MobSF（設定時） |
...
| Distribution protection | 🧪 Scripts / metadata | 🧪 TUF chain. SBOM, vulnerability, MobSF, and signature verification are BLOCKED until CI tooling and keys are provisioned | 🚧 Production keys and signed release process |
...
| Fuzz | Deterministic decoder / virtual BLE regression (Jazzer target is NOT_RUN) |
